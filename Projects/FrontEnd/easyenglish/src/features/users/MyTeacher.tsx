...
//#region AddedTeacher
interface UserRelationshipDisplay extends Users {
...
const MyTeacher = () => {
  ...
  const { data, isFetching, isLoading, isSuccess, isError, error } =
    useGetAllTeachersByUserQuery(loggedUser?.id as string);
  ...
  function convert2UserRelationshipDisplay(
    ...
    for (const row of data) {
      const { relatedUser, id, status, ...rest } = row;
  ...
  const deactivatedTeacher = (user: UserRelationshipDisplay) => {
    setCurrUser(user);
  ...
  return (
    <React.Fragment>
      ...
      {isView ? (
        <Box sx={{ width: "100%", p: 3 }}>
          <Paper sx={{ width: "100%", mb: 2, p: 2 }}>
            <EnhancedTableToolbar header="My Teacher" />
            <TableContainer>
              <Table
                sx={{ minWidth: 750 }}
                        ...
                        return (
                          <React.Fragment key={row.id}>
                            <TableRow>
                              ...
                              <TableCell align="right">
                                <Chip
                                  onClick={() => deactivatedTeacher(row)}
      ...
      <ConfirmationModal
        headerText={`${
          currUser?.relationshipStatus == Status.Active ? "Cancel" : "Active"
        } Teacher`}
        confirmationText={`Are you sure to  ${
          currUser?.relationshipStatus == Status.Active ? "cancel" : "active"
        } ${currUser?.aliasName} as your teacher ?`}
