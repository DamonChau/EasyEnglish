...
import { useGetExamTestQuery, useAddExamTestMutation, useUpdateExamTestMutation } from './examTests'
...
const ExamTestsDetail = () => {
    ...
    useEffect(() => {
        ...
        if (isEditing) {
            const { data, isFetching, isLoading, isSuccess, isError, error } = useGetExamTestQuery(id!)
    ...
    useEffect(() => {
        const r = JSON.stringify(convertToRaw(editorState.getCurrentContent()))
        const s = convertFromRaw(JSON.parse(r))
